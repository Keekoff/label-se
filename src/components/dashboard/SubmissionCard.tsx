      ...
      <CardContent className="p-6 bg-amber-200">
        <div className="space-y-6">
          ...
          <div className="flex gap-4">
            {paymentStatus === 'unpaid' && <Button onClick={onPayment} className="bg-primary hover:bg-primary-hover" disabled={isLoading}>
                <CreditCard className="mr-2 h-4 w-4" />
                {isLoading ? 'Chargement...' : 'Payer maintenant'}
              </Button>}
            <Button variant="outline" onClick={() => navigate(paymentStatus === 'paid' ? '/dashboard/justificatifs' : '/dashboard/payments')}>

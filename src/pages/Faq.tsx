...
const Faq = () => {
  ...
  const filteredItems = searchTerm.trim() === "" ? faqItems : faqItems.filter(item => item.question.toLowerCase().includes(searchTerm.toLowerCase()) || item.answer.toLowerCase().includes(searchTerm.toLowerCase()));
  return <div className="container mx-auto px-4 py-8 max-w-4xl animate-fadeIn">
      ...
      <div className="relative mb-8 max-w-md mx-auto">
        ...
        <Input type="text" placeholder="Rechercher une question..." className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg w-full focus:ring-[#35DA56] focus:border-[#35DA56]" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      </div>
      
      <Card className="p-6 shadow-md backdrop-blur-sm bg-white/95 border border-gray-100">
        <div className="space-y-4">
          {filteredItems.length > 0 ? filteredItems.map((item, index) => <FaqItem key={index} question={item.question} answer={item.answer} />) : <div className="text-center py-8">
